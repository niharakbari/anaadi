...
class SigLIPAdapter extends BaseAIAdapter {
  constructor() {
    super();
    this._session = null;
    this._isReady = false;
  ...
  async initialize() {
    ...
    try {
      ...
      this._isReady = true;
      logger.info(`SigLIPAdapter initialized successfully | model: ${manifest.paths.model} | dimension: ${manifest.dimension}`);
    } catch (error) {
      if (error instanceof AppError) throw error;
      throw new AppError(`Failed to initialize SigLIPAdapter: ${error.message}`, 500);
    }
  }
  ...
  async embed(imageBuffer) {
    ...
    try {
      ...
      const results = await this._session.run({ "pixel_values": inputTensor });
      const outputTensor = results["pooler_output"];
      
      if (!outputTensor) {
        throw new AppError(`embed(): ONNX session returned no output for key "pooler_output".`, 500);
      }
